...
# COMPITI CRLManager:
#   Gestione Full CRL (contiene tutti i certificati revocati)
#   Gestione Delta CRL (contiene solo le nuove revoche dall'ultimo aggiornamento)
#   Sincronizzazione tra Full e Delta
#   Pulizia automatica certificati scaduti
...
class CRLManager:
    ...
    def add_revoked_certificate(self, certificate, reason=ReasonFlags.unspecified):
        ...
        if any(entry["serial_number"] == serial_number for entry in self.revoked_certificates):
    ...
    def _cleanup_expired_certificates(self):
        """
        Rimuove certificati scaduti dalla lista revocati.
        
        Perché è importante:
        - Certificati scaduti non possono più essere usati
        - Non serve mantenerli nella CRL
        - Riduce dimensione CRL
        """
